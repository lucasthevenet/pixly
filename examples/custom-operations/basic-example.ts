...
const customSaturation = (factor: number): OperationFunction =>
  createOperation(
    async (bitmap, saturationFactor: number) => {
      const newData = new Uint8ClampedArray(bitmap.data);
      ...
      for (let i = 0; i < newData.length; i += 4) {
        ...
        newData[i] = Math.min(255, Math.max(0, avg + (r - avg) * saturationFactor));
        newData[i + 1] = Math.min(255, Math.max(0, avg + (g - avg) * saturationFactor));
        newData[i + 2] = Math.min(255, Math.max(0, avg + (b - avg) * saturationFactor));
      }
...
export async function runCustomOperationsExample() {
  try {
    ...
    console.log('Original image size:', processor.bitmap?.width, 'x', processor.bitmap?.height);
