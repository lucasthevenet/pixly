...
const convolution = (options: ConvolutionOptions): OperationFunction => {
  ...
  return createOperation(
    async (bitmap, params: ConvolutionOptions) => {
      ...
      for (let y = halfKernel; y < height - halfKernel; y++) {
        for (let x = halfKernel; x < width - halfKernel; x++) {
          for (let c = 0; c < 3; c++) { // RGB channels only
            ...
            newData[resultIndex] = Math.min(255, Math.max(0, sum / divisor + offset));
          }
        }
      }
...
const sobelEdgeDetection = (threshold: number = 100): OperationFunction =>
  createOperation(
    async (bitmap, threshold: number) => {
      const { width, height, data } = bitmap;
        ...
        for (let x = 1; x < width - 1; x++) {
          ...
          for (let ky = 0; ky < 3; ky++) {
            for (let kx = 0; kx < 3; kx++) {
              ...
              const luminance = data[pixelIndex] * 0.299 + 
...
const adaptiveContrastEnhancement = (windowSize: number = 64): OperationFunction =>
  createOperation(
    async (bitmap, windowSize: number) => {
      const { width, height, data } = bitmap;
      const newData = new Uint8ClampedArray(data);
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          ...
          const currentLuminance = data[currentIndex] * 0.299 + 
          ...
          newData[currentIndex] = Math.min(255, Math.max(0, data[currentIndex] * ratio));
          newData[currentIndex + 1] = Math.min(255, Math.max(0, data[currentIndex + 1] * ratio));
          newData[currentIndex + 2] = Math.min(255, Math.max(0, data[currentIndex + 2] * ratio));
        }
      }
        ...
        const chunkPromise = new Promise<void>((resolve) => {
          ...
          setTimeout(() => {
            ...
            for (let i = start; i < end; i += 4) {
              ...
              const factor = 1 + (intensity / 100) * Math.sin(luminance / 255 * Math.PI);
...
const mlStyleTransfer = (options: MLStyleOptions): OperationFunction =>
  createOperation(
    async (bitmap, params: MLStyleOptions) => {
      const { width, height, data } = bitmap;
      const newData = new Uint8ClampedArray(data);
  ...
  createConditionalOperation(
    ...
    createOperation(
      async (bitmap) => {
        ...
        const previewData = new Uint8ClampedArray(previewWidth * previewHeight * 4);
        ...
        const previewBitmap = new ImageData(previewData, previewWidth, previewHeight);
        ...
        for (let y = 0; y < bitmap.height; y++) {
          for (let x = 0; x < bitmap.width; x++) {
            ...
            const previewIndex = (Math.min(previewY, previewHeight - 1) * previewWidth + 
                                 Math.min(previewX, previewWidth - 1)) * 4;
