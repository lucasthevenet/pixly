...
describe("Function-based Operations", () => {
	it("should create operation functions that return functions", () => {
		const resizeOp = resize({ width: 100, height: 100, fit: "contain", position: "center", background: [0, 0, 0, 0] });
		const rotateOp = rotate(90, [0, 0, 0, 0]);
		const flipOp = flip("horizontal");
		const cropOp = crop({ x: 0, y: 0, width: 50, height: 50, background: [0, 0, 0, 0] });
		const blurOp = blur(5);
	...
	it("should apply function-based operations to processor", async () => {
		...
		const resizeOp = resize({ width: 4, height: 4, fit: "contain", position: "center", background: [0, 0, 0, 0] });
	...
	it("should handle operation errors gracefully", async () => {
		...
		await expect(applyOperation(processor, errorOperation))
			.rejects
			.toThrow("Operation failed: Test operation error");
	});
